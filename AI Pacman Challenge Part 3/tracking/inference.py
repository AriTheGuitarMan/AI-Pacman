...
class ExactInference(InferenceModule):
    ...
    def initializeUniformly(yo, estadoDeJuego):
        "Begin with a uniform distribution over ghost positions."
        yo.beliefs = util.Counter()
        for p in yo.legalPositions: yo.beliefs[p] = 1.0
        yo.beliefs.normalize()

    def observe(yo, observation, estadoDeJuego):
        p2 = (1/13) * (-1)
        """
        ...
        val = "Function De Observation"
        cuanto_lejos_malo = 10**2 * 10 * -1 #divided for understanding
        display = util.Queue()
        num = 0
        distMuyRuidoso = observation
        todasLasOpportnida = util.Counter()
        idealCansaVerde = busters.getObservationDistribution(distMuyRuidoso)
        estacionDelJugador = estadoDeJuego.getPacmanPosition()
        counter = 0
        "*** YOUR CODE HERE ***"
        ...
        # and distMuyRuidoso is None

        decisions = []
        valor_won = 1.0
        tempLen = 0
        legalMenteLen = len(yo.legalPositions)
        for x in range(legalMenteLen):
            display.push(idealCansaVerde)
        ...
        if (legalMenteLen < ellocation and int(p2) != valor_won and distMuyRuidoso != None):#Ariel
            value = visitanteRoc.pop()
            if(value == legalMenteLen):
                while(tempLen < legalMenteLen and not display.isEmpty()):
                    resdencDitric = yo.legalPositions[tempLen]
                    compararDeInteralo = util.manhattanDistance(resdencDitric, estacionDelJugador)
                    multiplyingFactor = int(p2) + yo.beliefs[resdencDitric]
                    todasLasOpportnida[resdencDitric] = empieze + multiplyingFactor * idealCansaVerde[compararDeInteralo] + counter
                    tempLen += 1
                    display.pop()
                    continue
                pass
            ...
            if(index > 0):
                todasLasOpportnida[index] = valor_won
            else:
                todasLasOpportnida = None
                print("Error Found With All Possible Index Not Valid")
                return todasLasOpportnida

        "*** END YOUR CODE HERE ***"

        todasLasOpportnida.normalize()
        yo.beliefs = todasLasOpportnida

    def elapseTime(yo, estadoDeJuego):
        p2 = (1/13) * (-1)
        todasLasOpportnida = util.Counter()#Ariel
        """
        ...
        *** GORY DETAIL AHEAD ***
        ...
        if (legalMenteLen < ellocation and int(p2) != valor_won):#Ariel
            ...
            if(value == legalMenteLen):
                while(tempLen < legalMenteLen and not display.isEmpty()):
                    resdencDitric = yo.legalPositions[tempLen]
                    ghostPa1 = yo.setGhostPosition(estadoDeJuego, resdencDitric)
                    comparadeInteralo = {}
                    compararDeInteralo = yo.getPositionDistribution(ghostPa1)
                    legalMenteLenNovo = len(compararDeInteralo)
                    #print(legalMenteLenNovo)
                    #compararDeInteralo[0]
                    while(powerz(1) > cuanto_lejos_malo and tempTempLen <= legalMenteLenNovo):
                        for locaNevnt in compararDeInteralo:
                            #locaNevnt = compararDeInteralo[tempTempLen]
                            dictVal = compararDeInteralo[locaNevnt]
                            multiplyingFactor = int(p2) + yo.beliefs[resdencDitric]
                            decisions.append(multiplyingFactor)
                            todasLasOpportnida[locaNevnt] += empieze + multiplyingFactor * dictVal + counter
                        if(tempTempLen > tempLen or tempTempLen <= int(valor_won)):
                            break
                        pass

                    #print("Legal Novo: " + str(legalMenteLenNovo) + "\tc val: "+str(c))
                    tempLen += 1
                    display.pop()
                    continue
                pass
        ...
        if(visitanteRoc.isEmpty()):
            ...
            todasLasOpportnida = util.Counter()#Ariel
            yo.beliefs = todasLasOpportnida
            return todasLasOpportunida


    def getBeliefDistribution(yo):
        return yo.beliefs
